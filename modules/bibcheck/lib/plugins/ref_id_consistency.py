...
def check_record(record):
	...
	for field_instance in record_get_field_instances(record,'999','C','5'):
		...
		if len(recid_set) > 1:
			record.set_invalid("Several records referenced by reference %s" % 
				''.join(field_get_subfield_values(field_instance, 'x')))


def find_inspire_id_from_reference(field_instance):
	"""
	Given a reference field instance returns a set of Inspire ids to which it's subfields refer to
	"""
	dictionary = {
		'a' : '0247_a',
		'i' : '020__a',
...
def find_rec_by_field_and_value(field_to_search,value): # todo add collection as param?
	"""
	Returns the first record that contains the given field with the given value
	"""
