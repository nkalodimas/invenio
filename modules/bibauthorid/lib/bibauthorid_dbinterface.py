...
def confirm_papers_to_author(pid, sigs_str, user_level=0):   ### confirm_papers_to_person
    ...
    for s in sigs_str:
        ...
        if not (author_not_rejected_papers or author_rejected_papers or diff_author_not_rejected_papers) or not sig_exists:
            statuses.append((False, 'confirm_failure'))
            continue
        statuses.append((True, 'confirm_success'))
...
def reject_papers_from_author(pid, sigs_str, user_level=0):   ### reject_papers_from_person
    ...
    for s in sigs_str:
        ...
        # assert sig_exists, 'The signature should exist'
        if not sig_exists:
            statuses.append((False, 'reject_failure'))
            continue
        statuses.append((True, 'reject_success'))
...
def reset_papers_of_author(pid, sigs_str):   ### reset_papers_flag
    ...
    for s in sigs_str:
        ...
        # assert sig_exists, 'The s should exist'
        if author_rejected_papers or not sig_exists:
            statuses.append((False, 'reset_failure'))
            continue
        statuses.append((True, 'reset_success'))
...
def get_claimed_papers_from_papers(recs):
    ...
    '''
    recs_sqlstr = _get_sqlstr_from_set(recs)
    claimed_recs = run_sql("""select bibrec
                              from aidPERSONIDPAPERS
                              where bibrec in %s
                              and flag=2"""
                              % recs_sqlstr )
...
def get_validated_request_tickets_for_author(pid, tid=None):   ### get_validated_request_ticket
    ...
    '''
    tickets = get_request_tickets_for_author(pid, tid)
    for entries, _ in list(tickets):
        for entry in list(entries):
            action, sig_str = entry
            if action in ['repeal', 'confirm']:   # These should be the only possible actions in a ticket!
                try:
                    sig = _split_signature_string(sig_str)
                    table, ref, rec = sig
    ...
    for ticket in list(tickets):
        entries, _ = ticket
        ticket_actions = [action for action, _ in entries]
        if 'repeal' not in ticket_actions and 'confirm' not in ticket_actions:
...
def update_request_ticket_for_author(pid, tag_value, tid=None):   ### update_request_ticket
    '''
    ...
    #       rt_external_id, rt_paper_cornfirm, rt_paper_reject, rt_paper_forget,
    #       rt_name, rt_email, rt_whatever
    # flag: rt_number

    if tid is None:
        last_tid = run_sql("""select max(opt1) from aidPERSONIDDATA
                              where personid=%s
                              and tag like %s""",
                              (str(pid), 'rt_%') )[0][0]
        tid = 1
        ...
        remove_request_ticket_for_author(pid, tid)

    for tag, value in tag_value:
        run_sql("""insert into aidPERSONIDDATA
                   (personid, tag, data, opt1)
                   values (%s, %s, %s, %s)""",
                   (str(pid), 'rt_' + str(tag), str(value), str(tid)) )
...
def get_request_tickets_for_author(pid, tid=None):   ### get_request_ticket
    ...
    return [[[(i[0][3:], i[1]) for i in tinfo], tid] for tid, tinfo in request_tickets]
...
def get_authors_with_open_tickets():   ### get_persons_with_open_tickets_list
    ...
    @rtype: tuple ((int, int),)
    '''
    return run_sql("""select personid, count(distinct opt1)
                      from aidPERSONIDDATA
                      where tag like %s
                      group by personid""",
                      ('rt_%',) )
    ...
    def format_record(record):
        ...
        if show_date:
            ...
            date_id = run_sql("""select id_bibxxx
                                 ...
                                 (rec,) )
            if date_id:
                date_id_sqlstr = _get_sqlstr_from_set(date_id, itemgetter(0))
                date = run_sql("""select value
                                  from bib26x
                                  where id in %s
                                  and tag=%s"""
                                  % (date_id_sqlstr, "'269__c'") )
                if date:
                    record_info['date'] = zip(*date)[0]

        if show_experiment:
            record_info['experiment'] = list()
            ...
            if experiment_id:
                experiment_id_sqlstr = _get_sqlstr_from_set(experiment_id, itemgetter(0))
                experiment = run_sql("""select value
                                        from bib69x
                                        where id in %s
                                        and tag=%s"""
                                        % (experiment_id_sqlstr, "'693__e'") )
                if experiment:
                    record_info['experiment'] = zip(*experiment)[0]
...
def _get_title_of_paper(rec):   ### get_title_from_rec
    '''
    ...
    @rtype: str
    '''
    title_ids = run_sql("""select id_bibxxx
                           from bibrec_bib24x
                           where id_bibrec=%s""",
                           (rec,) )

    if title_ids:
        title_ids_sql_str = _get_sqlstr_from_set(title_ids, lambda x: x[0])
...
def _get_doi_for_paper(recid):   ### get_doi_from_rec
    '''
    ...
    @rtype: str
    '''
    bib02x_ids = run_sql("""select id_bibxxx, field_number
                            from bibrec_bib02x
                            where id_bibrec=%s""",
                            (recid,) )
    if not bib02x_ids:
    ...
    bib02x_ids_sqlstr = _get_sqlstr_from_set(bib02x_ids, lambda x: x[0])
    doi = run_sql("""select id, tag, value
                     from bib02x
                     where id in %s"""
                     % bib02x_ids_sqlstr )
    if not doi:
    ...
    grouped = groupby(bib02x_ids, lambda x: x[1])
    doi_dict = dict((bib02x_id, (tag, value)) for bib02x_id, tag, value in doi)

    for group in grouped:
        bib02x_ids = [i[0] for i in list(group[1])]
        found = False
        code = None

        for bib02x_id in bib02x_ids:
            if doi_dict[bib02x_id][0] == '0247_2' and doi_dict[bib02x_id][1] == 'DOI':   ### or == 'doi'
                found = True
            elif doi_dict[bib02x_id][0] == '0247_a':
                code = doi_dict[bib02x_id][1]
...
def get_modified_papers_before(recs, before):   ### filter_modified_record_ids
    ...
    modified_recs = run_sql("select id from bibrec "
                            "where id in %s "
                             % recs_sqlstr +  "and modification_date < %s " , (before,) )
